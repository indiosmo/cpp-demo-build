...
lab::result<void> session::setup(const session_config& config, boost::asio::io_context* io_context)
{
  ...
  lab::match(config.decoder, [this](const auto& decoder_cfg) { setup_decoder(decoder_cfg); });
  ...
  LAB_LEAF_CHECK(
    lab::match(
      config.receiver,
      [this, io_context](const asio_udp_receiver_config& receiver_cfg) -> lab::result<void> {
        if (io_context == nullptr) {
          return lab::make_leaf_error(lab::error_code::configuration_error, "asio receiver requires a non-null io_context");
        }
        return setup_receiver(receiver_cfg, *io_context);
      },
      [this, io_context](const ef_vi_udp_receiver_config& receiver_cfg) -> lab::result<void> {
        if (io_context != nullptr) {
          return lab::make_leaf_error(lab::error_code::configuration_error, "ef_vi receiver does not take an io_context");
        }
...
lab::result<void> session::start()
{
  ...
  if (asio_receiver_) {
    LAB_LEAF_CHECK(asio_receiver_->start());
...
lab::result<void> session::setup_receiver(const asio_udp_receiver_config& config, boost::asio::io_context& io_context)
{
  ...
  receiver.on_datagram = [this](lab::network::types::datagram_view bytes) { session_->send(bytes); };

  return {};
}

lab::result<void> session::setup_receiver(const ef_vi_udp_receiver_config& config)
{
