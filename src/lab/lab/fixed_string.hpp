#ifndef LAB_FIXED_STRING_HPP
#define LAB_FIXED_STRING_HPP
...
public:
  ...
  static lab::result<fixed_string> from(std::string_view sv)
    requires(Policy == fixed_string_truncation_policy::strict)
  {
    if (sv.size() > N) {
      return lab::make_leaf_error(lab::error_code::out_of_bounds, "max size exceeded");
    }

    return from_string_view(sv);
  }

  [[nodiscard]] static lab::result<fixed_string> from(const char* data, std::size_t size)
    requires(Policy == fixed_string_truncation_policy::strict)
  {
    LAB_ASSERT(data != nullptr);
  ...
  [[nodiscard]] static lab::result<fixed_string> from(std::integral auto number)
  {
    fixed_string fs;
    ...
    if (result.size > N) {
      return lab::make_leaf_error(
        lab::error_code::out_of_bounds, fmt::format("number {} exceeds max string size {}", number, N));
    }
...
namespace std {
template <uint8_t N, lab::fixed_string_truncation_policy Policy>
struct hash<lab::fixed_string<N, Policy>>
{
  [[nodiscard]] std::size_t operator()(const lab::fixed_string<N, Policy>& fs) const noexcept
  {
    return boost::hash_range(fs.buffer_.begin(), fs.buffer_.end());
  }
};
...
template <uint8_t N, lab::fixed_string_truncation_policy Policy>
struct fmt::formatter<lab::fixed_string<N, Policy>>
{
  ...
  auto format(const lab::fixed_string<N, Policy>& str, FormatContext& ctx) const
  {
    return fmt::format_to(ctx.out(), "{0}", str.to_string_view());
  }
};

#endif /* LAB_FIXED_STRING_HPP */
