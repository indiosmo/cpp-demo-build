...
/*
 * Per-symbol order book: structural layer over engine-owned
 ...
 *
 * Templated on the side-map type so comparison fixtures can keep a std::map
 * reference shape near the production boost::container::flat_map book.
 */
...
using order_book = detail::basic_order_book<boost::container::flat_map>;
