 ...
 * cross-symbol resting_index_ map, plus any mbo_book_update emit the wire contract
 ...
 *     existing best bid. Each place takes the engine's full new_order_single path
 *     including the post-place mbo_book_update emit and the level walk that
 ...
 *     level. Each cancel emits a mbo_book_update whose total walks the surviving
 ...
 *     skips the mbo_book_update emit and the cancel runs at its minimum cost.
 *
 ...
 *     orders via on_market_data during the timed cross and re-places them in
...
const rt::types::security_id bench_security_id{1};
const rt::types::security_exchange bench_exchange{"BVMF"};
...
// Each seeded order owns a unique cl_ord_id; the client_id id cycles across a
// small pool to mimic many participants posting concurrently. Cycling rather
// than holding all order ids under one client_id id keeps the engine's
...
constexpr rt::types::client_id fresh_user{1};

struct seeded_order
{
  rt::types::client_id client_id;
  rt::types::cl_ord_id cl_ord_id;
  rt::types::side side;
  rt::types::price price;
  rt::types::quantity order_qty;
};
...
std::vector<seeded_order> build_seed_plan()
{
  ...
  auto emit_side = [&](rt::types::side side, std::uint64_t starting_price, bool descending) {
    for (std::size_t level_index = 0; level_index < levels_per_side; ++level_index) {
      ...
      for (std::size_t i = 0; i < orders_at_level; ++i) {
        const std::uint64_t order_qty = clamp_to<std::uint64_t>(quantity_distribution(rng), min_quantity, max_quantity);
        plan.push_back(
          seeded_order{
            .client_id = rt::types::client_id{(next_order_id % distinct_users) + 1},
            .cl_ord_id = rt::types::cl_ord_id{next_order_id},
            .side = side,
            .price = rt::types::price{level_price},
 ...
 * override on_market_data after construction; the rest leave it as a noop so the
...
struct bench_engine
{
  ...
  {
    engine.on_market_data = [](const md::message&) {};
    engine.on_order_entry = [](const rt::event&) {};
  }
...
rt::new_order_single to_new_order_single(const seeded_order& seed)
{
  return rt::new_order_single{
    .client_id = seed.client_id,
    .cl_ord_id = seed.cl_ord_id,
    .security_id = bench_security_id,
    .symbol = bench_symbol,
    .security_exchange = bench_exchange,
    .side = seed.side,
...
void seed(bench_engine& engine, const std::vector<seeded_order>& plan)
{
  for (const auto& entry : plan) {
    engine.send(to_new_order_single(entry));
  }
}
...
void BM_PlaceLimitNoFill(benchmark::State& state)
{
  ...
  // (existing level, no cross). Each place hits the engine's full new_order_single
  ...
  // mbo_book_update emit (own_best == price, so the engine emits and the
  ...
  // ids; one client_id owns all of them, which is cheaper than rotating users but
  ...
  for (std::size_t i = 0; i < batch_size; ++i) {
    const rt::types::cl_ord_id fresh_id{fresh_id_base + i};
    place_batch.push_back(
      rt::new_order_single{
        .client_id = fresh_user,
        .cl_ord_id = fresh_id,
        .security_id = bench_security_id,
        .symbol = bench_symbol,
        .security_exchange = bench_exchange,
        .side = rt::types::side::buy,
        .ord_type = rt::types::ord_type::limit,
        .time_in_force = rt::types::time_in_force::day,
        .order_qty = rt::types::quantity{1},
        .price = target_price,
      });
    cancel_batch.push_back(
      rt::cancel_order{
        .client_id = fresh_user,
        .cl_ord_id = fresh_id,
        .orig_cl_ord_id = rt::types::orig_cl_ord_id{fresh_id.get()},
      });
  }
...
void BM_PlaceCancelRoundTrip(benchmark::State& state)
{
  ...
  //
  // Reusing the same (client_id, cl_ord_id) across iterations is safe because each
  // iteration ends with the order cancelled and removed from resting_index_.
  const auto plan = build_seed_plan();
  bench_engine engine;
  seed(engine, plan);

  const rt::types::cl_ord_id roundtrip_id{fresh_id_base};
  const rt::request place_command = rt::new_order_single{
    .client_id = fresh_user,
    .cl_ord_id = roundtrip_id,
    .security_id = bench_security_id,
    .symbol = bench_symbol,
    .security_exchange = bench_exchange,
    .side = rt::types::side::buy,
    ...
    .time_in_force = rt::types::time_in_force::day,
    .order_qty = rt::types::quantity{1},
    .price = rt::types::price{base_bid_price},
  };
  const rt::request cancel_command = rt::cancel_order{
    .client_id = fresh_user,
    .cl_ord_id = roundtrip_id,
    .orig_cl_ord_id = rt::types::orig_cl_ord_id{roundtrip_id.get()},
  };
...
// the run for the at-top fixture), keeping the mbo_book_update level walk a
// comparable workload.
...
void prepare_cancel_fixture(cancel_at_level_fixture& fixture, rt::types::side side, rt::types::price target_price)
{
  ...
  for (std::size_t i = 0; i < cancel_slab_headroom; ++i) {
    const rt::types::cl_ord_id cl_ord_id{next_id++};
    fixture.engine->send(
      rt::new_order_single{
        .client_id = fresh_user,
        .cl_ord_id = cl_ord_id,
  ...
  for (std::size_t i = 0; i < cancel_iterations_per_repetition; ++i) {
    ...
    fixture.cancels.push_back(
      rt::cancel_order{
        .client_id = fresh_user,
        .cl_ord_id = cl_ord_id,
        .orig_cl_ord_id = rt::types::orig_cl_ord_id{cl_ord_id.get()},
      });
  }
}
...
void BM_CancelAtTopOfBook(benchmark::State& state)
{
  ...
  // the engine emits a mbo_book_update whose total_at_best_bid walks the level's
  // surviving intrusive list. This exercises the cancel hot path that real
  // book-update churn pays.
...
void BM_CancelDeepInBook(benchmark::State& state)
{
  ...
  // mbo_book_update emit and the cancel runs at its minimum cost: variant
...
void setup_cross(const benchmark::State&)
{
  ...
  for (const auto& entry : plan) {
    if (entry.side != rt::types::side::sell) {
      continue;
    }
    ask_levels[entry.price].push_back(entry);
  }
  ...
  for (const auto& [level_price, level_orders] : ask_levels) {
    for (const auto& level_order : level_orders) {
      marketable_quantity += level_order.order_qty.get();
    }
  ...
  for (std::size_t i = 0; i < cross_iterations_per_repetition; ++i) {
    g_cross_fixture.marketable_commands.push_back(
      rt::new_order_single{
        .client_id = fresh_user,
        .cl_ord_id = rt::types::cl_ord_id{fresh_id_base + i},
        .security_id = bench_security_id,
        .symbol = bench_symbol,
        .security_exchange = bench_exchange,
        .side = rt::types::side::buy,
        .ord_type = rt::types::ord_type::limit,
        .time_in_force = rt::types::time_in_force::day,
        .order_qty = rt::types::quantity{marketable_quantity},
        .price = marketable_limit,
      });
  }
}
