# 2. Use CMake `FetchContent` for third-party dependencies
...
[`0002-use-fetchcontent-for-third-party-dependencies-matrix.html`](0002-use-fetchcontent-for-third-party-dependencies-matrix.html).
...
used by domain code and tests (`fmt`, `spdlog`, `Catch2`, and similar). Every
third-party dependency in this layer is in scope; nothing in it is taken from
system packages.
...
buildable trees for compiled libraries (`fmt`, `Catch2`, and similar), which
inflates the repository and turns updates into multi-step recopy operations
driven by an out-of-band script. The split arrangement is rejected
...
6. For libraries of functions whose canonical target name is already
   ...
   `spdlog::spdlog`), no `lab::` wrapper is added; consumers link the
