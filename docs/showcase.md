...
[`kraken_submission::application`](../submission/src/kraken_submission/src/application.cpp)
...
[`matching_engine::v3::engine::handle(new_order)`](../submission/src/matching_engine/src/v3/engine.cpp)
-- compiles only if every named field receives the correct strong
type.

[`strong_type_test.cpp`](../submission/test/kraken/src/strong_type_test.cpp)
...
([`csv_encoder.cpp`](../submission/src/market_data/src/csv_encoder.cpp));
adding a new event alternative without an `encode_*` branch fails to
compile here. The engine's command dispatch in
[`engine.cpp`](../submission/src/matching_engine/src/v3/engine.cpp)
(`engine::send` -> `kraken::match` -> `handle(...)` overloads) enforces
...
[`application::start`](../submission/src/kraken_submission/src/application.cpp)
uses `KRAKEN_LEAF_CHECK` for the socket bind and each loop's
`start()`. The matching engine's public `send` / `handle` overloads
return `void`: their `kraken::result`-bearing helpers
(`handle_new_order_impl`, `find_book`, `check_duplicate`) collapse
failures into `boost::leaf::try_handle_all` at the public boundary,
matching the narrow-contract stance in [`../DESIGN.md`](../DESIGN.md).
Cross-thread posting goes through
[`kraken::event_loop::post`](../submission/src/kraken/kraken/event_loop.hpp),
...
[`kraken/log.hpp`](../submission/src/kraken/kraken/log.hpp) is the
shared surface for one concern that cuts across runtime components:
diagnostic logging. The `KRAKEN_LOG_*` macros capture source
...
[`application::configure_logger`](../submission/src/kraken_submission/src/application.cpp).
...
concern stays behind a small `kraken::` utility.

## [Declarative style](cpp-design-principles.md#declarative-style)

[`engine::handle(new_order)`](../submission/src/matching_engine/src/v3/engine.cpp)
...
predicate in
[`matching.hpp`](../submission/src/matching_engine/matching_engine/v3/matching.hpp)
...
[`matching_engine::order_book`](../submission/src/matching_engine/matching_engine/order_book.hpp)
alias). A head-to-head
[benchmark](../submission/benchmarks/order_book/src/matching_engine_order_book_benchmark.cpp)
...
[`matching_engine_test.cpp`](../submission/test/matching_engine/src/matching_engine_test.cpp)
...
- `static_assert` blocks pin constructibility, conversion, and
  ...
  `kraken::FixedString<fs_strict>` confirms the concept matches the
  template.
...
- A small `capture_error_code` helper adapts `kraken::result` into a
  `std::error_code` so error paths can be compared directly against
  `kraken::error_code::out_of_bounds` instead of unpacking leaf
