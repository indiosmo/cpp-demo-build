...
// IMPROVEMENT: collapsing the inline side switch requires lifting side to a
// compile-time parameter. See DESIGN.md.
...
{
  // IMPROVEMENT: a production protocol would emit rejects here;
  // the exercise spec has no reject event, so we drop.
  boost::leaf::try_handle_all(
    [&] { return handle_new_order_impl(request); },
    [&](kraken::match_error<errors::duplicate_order>) {
      KRAKEN_LOG_WARN(
        "rejecting new_order: duplicate (user={}, user_order_id={}) already resting", request.user, request.order_id);
    },
    [&](kraken::match_error<errors::unknown_symbol>) {
      KRAKEN_LOG_WARN("rejecting new_order: unknown symbol {}", request.instrument);
    },
    [](const kraken::error& err) { KRAKEN_LOG_ERROR("unhandled engine error: {}", err.full_details()); },
    [] { KRAKEN_LOG_ERROR("unhandled engine error: unknown failure"); });
}

kraken::result<void> engine::handle_new_order_impl(const rt::new_order& request)
{
  ...
  if (should_place_order) {
    ...
    // in a kraken::scope_guard-backed helper so partial progress rolls back.
...
kraken::result<void> engine::check_duplicate(const types::order_key& key) const
{
  ...
  if (resting_index_.contains(key)) {
    return kraken::make_leaf_error(errors::duplicate_order{.user = key.user, .order_id = key.order_id});
  }
  return {};
}

kraken::result<flat_order_book*> engine::find_book(rt::types::symbol instrument)
{
  ...
  if (book_it == books_.end()) {
    return kraken::make_leaf_error(errors::unknown_symbol{.instrument = instrument});
  }
...
void engine::handle(const rt::cancel_order& incoming)
{
  ...
  if (index_it == resting_index_.end()) {
    KRAKEN_LOG_WARN("cancel miss: user={} order_id={}", incoming.user, incoming.order_id);
  ...
  KRAKEN_ASSERT(book_it != books_.end());
