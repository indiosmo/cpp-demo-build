 ...
 *
 * send() dispatches through kraken::match to the typed handle()
...
private:
   ...
   */
  kraken::result<void> handle_new_order_impl(const order_routing::new_order&);

  kraken::result<void> check_duplicate(const types::order_key& key) const;
  kraken::result<flat_order_book*> find_book(order_routing::types::symbol instrument);
