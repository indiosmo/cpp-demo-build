...
depend on `kraken::` aliases, never on the upstream header directly.
...
| `charconv` | `from_chars` / `to_chars` returning `kraken::result`, with exact and partial match modes and built-in support for `strong_type` wrappers. |
| `hash` | Reflective `auto_hash` for aggregates via `boost::pfr`, plus the `KRAKEN_STD_HASH` macro that wires it into `std::hash`. |
