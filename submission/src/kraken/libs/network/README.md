...
codebase depends on `kraken::network::` symbols rather than on Boost.Asio
...
in `kraken/network/types.hpp`; see the receiver headers for the
...
- `kraken::network::asio` -- pulls in Boost.Asio header-only and confines
...
- `kraken::network::ef_vi` -- stand-in for the Solarflare SDK target. A
