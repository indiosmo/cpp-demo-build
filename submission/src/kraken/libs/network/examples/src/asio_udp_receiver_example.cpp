/*
 * Standalone example for kraken::network::asio_udp_receiver. Binds a UDP
...
kraken::result<example_config> parse_args(int argc, char** argv)
{
  ...
  if (argc >= 3) {
    ...
    if (parse_result.ec != std::errc{} || parse_result.ptr != port_end) {
      return kraken::make_leaf_error(kraken::error_code::configuration_error, fmt::format("invalid port '{}'", port_argument));
    }
...
int main(int argc, char** argv)
{
  return boost::leaf::try_handle_all(
    [&]() -> kraken::result<int> {
      ...
      KRAKEN_LEAF_CHECK(receiver.start());
    ...
    KRAKEN_RESULT_CATCH_ALL(return EXIT_FAILURE;));
}
