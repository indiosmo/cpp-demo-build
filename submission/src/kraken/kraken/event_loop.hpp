#ifndef KRAKEN_EVENT_LOOP_HPP
#define KRAKEN_EVENT_LOOP_HPP

#include "kraken/assert.hpp"
#include "kraken/concurrent_queue.hpp"
...
{
public:
  using task_t = kraken::inplace_function<void(), 2048>;
  using poller_t = kraken::inplace_function<bool(), 128>;
  ...
  kraken::result<void> start()
  {
    if (running_.load(std::memory_order_acquire)) {
      return kraken::make_leaf_error(kraken::error_code::already_in_progress, "event loop already running");
    }
    ...
    try {
      ...
      return kraken::make_leaf_error(kraken::error_code::generic_error, ex.what());
    }
  ...
  {
    KRAKEN_ASSERT(task_queue_.enqueue(std::move(task)));
  }
...
private:
  void run()
  {
    ...
    KRAKEN_LOG_INFO("event loop started: {}", config_.thread_name);
    ...
    KRAKEN_LOG_INFO("event loop stopped: {}", config_.thread_name);
  }
  ...
  static void set_current_thread_name(std::string_view name)
  {
    ...
    if (pthread_setname_np(pthread_self(), truncated.c_str()) != 0) {
      KRAKEN_LOG_WARN("failed to set thread name: {}", name);
    }
  }
