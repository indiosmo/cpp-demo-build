#ifndef KRAKEN_LOG_HPP
#define KRAKEN_LOG_HPP
 ...
 * startup. See DEPENDENCIES.md for why kraken does not take spdlog directly.
 */
...
struct console_logger
{
  ...
  {
    // stdout is the market-data stream the grader diffs; diagnostics stay on stderr.
    fmt::print(stderr, "{}\n", line);
  }
};
...
#define KRAKEN_LOG_TRACE(...)    ::kraken::log<::kraken::log_level::trace>(std::source_location::current(), __VA_ARGS__)
#define KRAKEN_LOG_DEBUG(...)    ::kraken::log<::kraken::log_level::debug>(std::source_location::current(), __VA_ARGS__)
#define KRAKEN_LOG_INFO(...)     ::kraken::log<::kraken::log_level::info>(std::source_location::current(), __VA_ARGS__)
#define KRAKEN_LOG_WARN(...)     ::kraken::log<::kraken::log_level::warn>(std::source_location::current(), __VA_ARGS__)
#define KRAKEN_LOG_ERROR(...)    ::kraken::log<::kraken::log_level::error>(std::source_location::current(), __VA_ARGS__)
#define KRAKEN_LOG_CRITICAL(...) ::kraken::log<::kraken::log_level::critical>(std::source_location::current(), __VA_ARGS__)

#endif /* KRAKEN_LOG_HPP */
