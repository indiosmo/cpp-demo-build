#ifndef KRAKEN_ALGORITHM_HPP
#define KRAKEN_ALGORITHM_HPP
...
/*
 * string_view algorithms shared across the kraken vocabulary. Trimming and
 ...
 * KRAKEN_ASSERT. Returned views slice into body (which must outlive them)
...
std::array<std::string_view, N> split_fields(std::string_view body, char delimiter = ',')
{
  ...
  KRAKEN_ASSERT(parts.size() == N);

  std::array<std::string_view, N> fields{};
  for (std::size_t i = 0; i < N; ++i) {
    fields[i] = kraken::trim(parts[i]);
  }
  return fields;
}
