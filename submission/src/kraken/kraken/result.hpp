#ifndef KRAKEN_RESULT_HPP
#define KRAKEN_RESULT_HPP
 ...
 * lambda; CATCH_ALL chains the three in order (kraken::error, std::exception,
 * then any std::error_code). code_block runs after the diagnostic write,
 * typically returning a failure value.
 */
#define KRAKEN_RESULT_CATCH_EXCEPTION(code_block)                                                          \
  [&]([[maybe_unused]] const std::exception& ex, [[maybe_unused]] const boost::leaf::diagnostic_info& diag) { \
    std::fprintf(stderr, "unhandled exception | %s\n", ex.what());                                         \
    code_block                                                                                             \
  },

#define KRAKEN_RESULT_CATCH_KRAKEN_ERROR(code_block)                                                       \
  [&]([[maybe_unused]] const kraken::error& err, [[maybe_unused]] const boost::leaf::diagnostic_info& diag) { \
    std::fprintf(stderr, "unhandled kraken::error | %s\n", err.full_details().c_str());                    \
    code_block                                                                                             \
  },

#define KRAKEN_RESULT_CATCH_ALL(code_block)                                                                \
  KRAKEN_RESULT_CATCH_KRAKEN_ERROR(code_block)                                                             \
  KRAKEN_RESULT_CATCH_EXCEPTION(code_block)                                                                \
...
#define KRAKEN_RESULT_DISCARD_ERRORS() [](){}

#define KRAKEN_RESULT_TOKEN_PASTE(x, y, z)  x ## y ## z
#define KRAKEN_RESULT_TOKEN_PASTE3(x, y, z) KRAKEN_RESULT_TOKEN_PASTE(x, y, z)
#define KRAKEN_RESULT_TMP(prefix)           KRAKEN_RESULT_TOKEN_PASTE3(prefix, _boost_leaf_tmp_, __LINE__)
...
#define BOOST_LEAF_CHECK(r) KRAKEN_LEAF_CHECK(r)

/*
 * BOOST_LEAF_ASSIGN variant that annotates the failure path with an extra
 * kraken::error before propagating, so handlers see both the inner cause and
 * the contextual wrapper.
 */
#define KRAKEN_RESULT_ASSIGN_ERROR(v, r, ...)                                                                 \
  auto&& BOOST_LEAF_TMP = r;                                                                                  \
  {                                                                                                           \
    static_assert(::boost::leaf::is_result_type<typename std::decay<decltype(BOOST_LEAF_TMP)>::type>::value);  \
    if (!BOOST_LEAF_TMP) {                                                                                    \
      auto KRAKEN_RESULT_TMP(kraken) = boost::leaf::on_error(kraken::error{__VA_ARGS__});                     \
      return boost::leaf::new_error(BOOST_LEAF_TMP.error());                                                  \
    }                                                                                                         \
  }                                                                                                           \
  v = std::forward<decltype(BOOST_LEAF_TMP)>(BOOST_LEAF_TMP).value()

#define KRAKEN_RESULT_CHECK_ERROR(r, ...)                                                                     \
  {                                                                                                           \
    auto&& BOOST_LEAF_TMP = (r);                                                                              \
    static_assert(::boost::leaf::is_result_type<typename std::decay<decltype(BOOST_LEAF_TMP)>::type>::value);  \
    if (!BOOST_LEAF_TMP) {                                                                                    \
 ...
 * with KRAKEN_RESULT_CATCH_ALL (which logs through stderr). Building block
 * for the top-level "fail loud and exit" helpers below.
 */
#define KRAKEN_RESULT_TO_BOOL(func)                                                                           \
  [&]() -> bool {                                                                                             \
    return boost::leaf::try_handle_all(                                                                       \
      [&]() -> kraken::result<bool> {                                                                         \
        KRAKEN_LEAF_CHECK(func);                                                                              \
        return true;                                                                                          \
      },                                                                                                      \
      KRAKEN_RESULT_CATCH_ALL(return false;));                                                                \
  }()

/*
 * Consume a result-returning call at main(): on failure, log via
 * KRAKEN_RESULT_CATCH_ALL and return EXIT_FAILURE from the enclosing function.
 */
#define KRAKEN_EXIT_ON_ERROR(func) \
  if (!KRAKEN_RESULT_TO_BOOL(func)) { return EXIT_FAILURE; }

/*
 * Test-only BOOST_LEAF_ASSIGN that REQUIREs success, so the test stops at the
 * offending call instead of cascading.
 */
#define KRAKEN_REQUIRE_LEAF(v, r)                                                                             \
  auto&& BOOST_LEAF_TMP = boost::leaf::try_handle_some(                                                       \
    [&]() { return r; },                                                                                      \
    KRAKEN_RESULT_CATCH_ALL(return boost::leaf::new_error();)                                                 \
  );                                                                                                          \
 ...
 */
template <typename T>
  requires kraken::LeafDiagnostic<T>
