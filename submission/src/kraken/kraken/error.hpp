#ifndef KRAKEN_ERROR_HPP
#define KRAKEN_ERROR_HPP
 ...
 * type carried inside a kraken::error.
 */
template <ErrorData T>
struct match_error
{
...
{
  return make_leaf_error(kraken::errors::generic_error{.ec = ec, .text{std::move(text)}}, loc);
}
...
template <kraken::ErrorData T>
struct is_predicate<kraken::match_error<T>> : std::true_type
{
};

template <kraken::ErrorData... Ts>
struct is_predicate<kraken::match_errors<Ts...>> : std::true_type
{
};

} // namespace boost::leaf

#endif /* KRAKEN_ERROR_HPP */
