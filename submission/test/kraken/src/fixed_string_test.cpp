...
using fs_strict = kraken::fixed_string<5>;
using fs_auto = kraken::fixed_string<5, kraken::fixed_string_truncation_policy::auto_truncate>;
using fs_large = kraken::fixed_string<8>;
...
std::error_code capture_error_code(Fn&& fn)
{
  return boost::leaf::try_handle_all(
           [&]() -> kraken::result<std::optional<std::error_code>> {
           ...
           [](const kraken::error& err) { return std::optional{err.error_code()}; },
...
static_assert(kraken::is_fixed_string_v<fs_strict>);
static_assert(kraken::is_fixed_string_v<fs_auto>);
static_assert(kraken::FixedString<fs_strict>);
static_assert(!kraken::FixedString<std::string>);

static_assert(std::is_same_v<kraken::truncating_fixed_string<5>, fs_auto>);
...
TEST_CASE("fixed_string - strict string factories")
{
  ...
  {
    KRAKEN_REQUIRE_LEAF(auto value, fs::from(std::string_view{"12345"}));
    CHECK(value == "12345");
  ...
  SECTION("from string_view overflow")
  {
    ...
    CHECK(ec == kraken::error_code::out_of_bounds);
  }

  SECTION("from pointer and size success")
  {
    char raw[6]{'1', '2', '3', '4', '5', '6'};
    KRAKEN_REQUIRE_LEAF(auto value, fs::from(raw, 5));
    CHECK(value == "12345");
    CHECK(!value.truncated());
  }
...
TEST_CASE("fixed_string - integral factory")
{
  ...
  SECTION("success paths")
  {
    ...
    KRAKEN_REQUIRE_LEAF(auto value, fs::from(input));
  ...
  SECTION("roundtrip")
  {
    using clordid = kraken::fixed_string<40>;
    const long value = 265944835702835;
    KRAKEN_REQUIRE_LEAF(auto str, clordid::from(value));
...
TEST_CASE("fixed_string - auto_truncate constructors")
{
  ...
  {
    kraken::truncating_fixed_string<5> value{"123456"};
    CHECK(value == "12345");
    CHECK(value.truncated());
  }
}
...
{
  using fs = kraken::fixed_string<5>;
  ...
  KRAKEN_REQUIRE_LEAF(auto value, fs::from(std::string_view{input}));
  CHECK(value.remove_suffix(suffix) == expected);
}

TEST_CASE("fixed_string - cross-size copy/move propagates truncated")
{
  using fs5 = kraken::fixed_string<5>;
  using fs8 = kraken::fixed_string<8>;
  using fs5_auto = kraken::fixed_string<5, kraken::fixed_string_truncation_policy::auto_truncate>;
